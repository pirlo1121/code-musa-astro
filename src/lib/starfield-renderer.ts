...
// Drawing is one drawImage/fillRect per visible star, using sprites
// pre-rendered once.
//
// This module touches no DOM APIs, so the same renderer runs either inside a
// Web Worker on an OffscreenCanvas (preferred: keeps all drawing off the main
// thread, so it never competes with scrolling) or on the main thread as a
// fallback. starfield.ts decides which and drives it with StarfieldMessage.
...
export type StarfieldMessage =
  | { type: 'init'; canvas: OffscreenCanvas; isMobile: boolean }
  | { type: 'resize'; width: number; height: number; dpr: number }
  | { type: 'pointer'; x: number; y: number }
  | { type: 'state'; running: boolean; reducedMotion: boolean };

export interface StarfieldRenderer {
  resize(cssWidth: number, cssHeight: number, dpr: number): void;
  setPointer(x: number, y: number): void;
  setState(running: boolean, reducedMotion: boolean): void;
}

export function applyMessage(renderer: StarfieldRenderer, msg: StarfieldMessage): void {
  if (msg.type === 'resize') renderer.resize(msg.width, msg.height, msg.dpr);
  else if (msg.type === 'pointer') renderer.setPointer(msg.x, msg.y);
  else if (msg.type === 'state') renderer.setState(msg.running, msg.reducedMotion);
}

type AnyCanvas = HTMLCanvasElement | OffscreenCanvas;
type Ctx2D = CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D;

// Works in both window and worker scopes (rAF exists in dedicated workers in
// Chromium and Firefox; older Safari workers fall back to a timer).
...
const cancelFrame = (id: number) => (hasRaf ? cancelAnimationFrame(id) : clearTimeout(id));

function createCanvas(size: number): AnyCanvas {
...
function makeSprite(rgb: RGB, spikes: boolean): AnyCanvas {
...
export function createStarfieldRenderer(canvas: AnyCanvas, isMobile: boolean): StarfieldRenderer | null {
  const ctx = canvas.getContext('2d') as Ctx2D | null;
  if (!ctx) return null;

  // Phones: fewer far stars (most are off-screen anyway) and a 30fps cap —
  // the orbit moves a fraction of a pixel per frame, so 30 vs 60fps is not
  // visible, but it halves the GPU work while the page is scrolling.
  const NEAR_COUNT = isMobile ? 260 : 620;
  const FAR_COUNT = isMobile ? 1200 : 3600;
  ...
  return {
    ...
    setState(running, reducedMotion) {
